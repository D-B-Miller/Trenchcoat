...
def estPlasmaSize(path,Tlim=800.0,cmap='hot',frames='all',return_data=False,**kwargs):
    '''
        ...
        Inputs:
            ...
            Tlim : Temperature limit in degrees C. Default 800
    ...
    ax.plot(time,area,'x-')
    # set the labels
    ax.set(xlabel="Time (s)",ylabel="Contour Area $(pix^2)$",title=kwargs.get("title",f"{os.path.splitext(os.path.basename(path))[0]} Est. Plasma Area vs Time"))
    return f,stack_imgs


def collectPlasmaStats(path: str,Tlim: float = 800.0,**kwargs) -> pd.DataFrame:
    '''
        Collect statistics about the area identified in the plasma.

        Based off estPlasmaSize, the temperature is masked to those above Tlim and the contour around the edge is drawn.
        The contour is used to create a masked area from which several statistics are collected.
        ...
        These statistics are combined together into a dataframe with appropriate column names.
    ...
    for tlim in Tlim:
        for i in range(nf):
            ...
            if len(cts)>0:
                ...
                meanT.append(mask_arr.mean())
                minT.append(mask_arr.min())
                maxT.append(mask_arr.max())
                height.append(h)
                width.append(w)
                asprat.append(w/h)
            else:
    ...
    return pd.DataFrame({'Frame Index':index,'Time (s)':time,'Contour Area (pix^2)':area,'Variance (deg C)':var,
                         'Std Dev (deg C)':std_dev,'Mean Temperature (deg C)':meanT,'Min. Temperature (deg C)':minT,
                         'Max. Temperature (deg C)':maxT,'Width (pix)':width,'Height (pix)':height,'Aspect Ratio (Width/Height)':asprat,"Temperature Threshold (deg C)":temp_limit})


def combinePlasmaStats(path,Tlim=800.0,**kwargs):
    '''
        Applied collectPlasmaStats to each of the files found in the wildcard path and then stacks them together into a giant dataframe
    ...
    labels = kwargs.get("labels",[os.path.splitext(os.path.basename(fn))[0] for fn in path])
    all_dfs = []
    for fn,label in zip(path,labels):
        # collect statistics using target settings
        df = collectPlasmaStats(fn,Tlim,**kwargs)
        # store the file path
        df['File Path'] = fn
...
def findTrueArea(df):
    '''
        Processes the area values in the dataframe from combinePlasmaStats to find the true area between the thresholds.
    ...
    for lb in df['Label'].unique():
        ...
        for ta,tb in zip(ths,ths[1:]):
            ...
            #larger_sz = max([ta,tb],key=lambda x:filt[filt['Temperature Threshold (deg C)']==x].shape[0])
            #ref_time=filt[filt['Temperature Threshold (deg C)']==larger_sz]['Time (s)'].values
...
def estAllPlasmaSize(path,Tlim=800.0,**kwargs):
    '''
        ...
        As there's usually an offset between the traces due to when the plasma is started, they won't be aligned.
        ...
        Inputs:
            ...
            Tlim : Temperature threshold in degrees C. Default 800.0
    ...
    if len(kwargs.get("labels",[]))>0:
        ...
        labels = [os.path.splitext(os.path.basename(fn))[0] for fn in path]
    # ensure it is a list
    if isinstance(Tlim,(float,int)):
        Tlim = [Tlim,]
        ...
        for tl in Tlim:
            ...
            for i in range(nf):
                ...
                if len(cts)>0:
                    ...
                    time.append(i/30.0)
                    index.append(i)
            #print(f"area {len(area)}, time {len(time)}, index {len(index)}")
    ...
    f.suptitle(kwargs.get("title",f"Estimated Plasma Area vs Time (Tlim={Tlim}"+"$^\circ$C)"))
    ax.legend()
    # return results
...
def fitPolyToPlasmaArea(data: str,**kwargs) -> tuple[plt.Figure, pd.DataFrame]:
    '''
        Using the data from estAllPlasmaSize, fit a 1D line to a specific time period of each data.
    ...
    if 't' in kwargs:
        ...
        if isinstance(kwargs['area'],(list,tuple)):
            areaA,areaB = kwargs['area']
            if ((areaA>=0) and (areaA<=1)) and ((areaB>=0) and (areaB<=1)):
                use_rel_area = True
                suffix=f"rel. area=[{areaA},{areaB}]"
            else:
                use_area = True
                suffix=f"area=[{areaA},{areaB}]"
        else:
    ...
    #new_ticks = kwargs.get("new_ticks",list(set([d['label'] for fn,v in data.items() for t,d in v.items()])))
    # gradient value
    grad = []
    off = []
    # max area
    ...
    for t,v in new_dict.items():
        ...
        for fn,d in v.items():
            ...
            if use_time:
                if isinstance(kwargs['t'],dict):
                    tA,tB = kwargs['t'][fn]
                # make to time period
                mask = np.where((pts[:,1]>=tA) & (pts[:,1]<=tB))[0]
            # extract area period
            elif use_area:
                if isinstance(kwargs['area'],dict):
                    areaA,areaB = kwargs['area'][fn]
                # else use regular area col
                else:
                    mask = np.where((pts[:,2]>=areaA) & (pts[:,2]<=areaB))[0]
                # apply additional mask to only used masked data within a 1 sec period
                #tmin = pts[mask,1].max()-pts[mask,1].min()
                ...
                if ((areaA>=0) and (areaA<=1)) and ((areaB>=0) and (areaB<=1)):
                    mask = np.where((pts[:,3]>=areaA) & (pts[:,3]<=areaB))[0]
                # else use regular area col
                else:
                    mask = np.where((pts[:,3]>=areaA) & (pts[:,3]<=areaB))[0]
                mask = mask[np.where(pts[mask,1] <= (pts[mask,1].min()+kwargs.get("tp",5.0)))[0]]
            ...
            print(d['label'],xp.min(),xp.max(),f"p={xp.max()-xp.min()}")
            yp = np.poly1d(z)(xp)
            offset=0
            if kwargs.get("use_reltime",True):
                offset = xp.min()
        ...
        df = pd.DataFrame({f"Gradient ({suffix})":grad,f"Offset ({suffix})":off,f"Max Area ({suffix}":maxm})
        df['File'] = list(v.keys())
        df['Label'] = [d['label'] for d in v.values()]
        df['Threshold (C)'] = t
        dfs.append(df)
        ...
        ff.suptitle(kwargs.get("fit_title",f"Fitted Lines to Plasma Contour Area\n{suffix} T>={t}"+ "$^\circ$C"))
        ...
        f.suptitle(kwargs.get("title",f"Plasma Contour Area vs Time with Fitted Lines\n{suffix} T>={t}"+ "$^\circ$C"))
        ...
        # set the title
        fz.suptitle(kwargs.get("gradient_title",f"Plasma Contour Area Fitted Line Paramaters {suffix}\nT>={t}"+ "$^\circ$C"))
        ...
        fm.suptitle(f"Max Plasma Area ($pix^2$) vs Plasma Gas Flow Rate (SL/min)\nT>={t}"+ "$^\circ$C")
        ...
        fmn.suptitle(f"Average Plasma Area ($pix^2$) vs Plasma Gas Flow Rate (SL/min)\nT>={t}"+ "$^\circ$C")
        # update figure dict
...
def makeColorBar(cmap: str,width: int,height: int,T: bool = False) -> np.ndarray:
...
def convertTempToVideo(data: np.ndarray,opath: str = None,cmap: int = cv2.COLORMAP_HOT):
    '''
...
def recolormapCsplit(data: str | np.ndarray,csplit: int,opath: str,cmap: int = cv2.COLORMAP_HOT):
    '''
    ...
    for ff in range(nf):
        ...
        if cmap == "gray":
            frame = cv2.hconcat((A,B))
            out.write(frame)
        # else apply colormap to the data
        else:
            A_img = cv2.applyColorMap(A,cmap)
            B_img = cv2.applyColorMap(B,cmap)
            frame = cv2.hconcat([A_img,B_img])
            # write data to video file
            out.write(frame)
    # release video writer
    out.release()
...
def recolormapSections(data: str | np.ndarray,secs: int,opath: str,cmap: int = cv2.COLORMAP_HOT,axis: int = 1):
    '''
        Colormap the target data normalizing the data in two parts specified by csplit
    ...
    if axis ==0:
        ...
        raise ValueError(f"Axis selection has to be either 0 or 1! Received {axis}!")
    # get shape of data
    nf,rows,width = data.shape
    # create fourcc
    fourcc = cv2.VideoWriter_fourcc(*'mjpg')
...
def pltTempToVideo(data: str | np.ndarray,opath: str,cmap: str = 'hot'):
    '''
...
def drawContoursVideo(data: np.ndarray,opath: str = None,cmap: int = cv2.COLORMAP_HOT,color: tuple[int, int, int] = (0,255,0)):
    '''
        Create a video drawing contours around the found objects in the temperature
        data.
...
def binFrame(data: np.ndarray,levels: int = 5) -> np.ndarray:
    ...
    # to its index
    for bi,(bA,bB) in enumerate(zip(edges,edges[1:])):
        binned[(data>=bA)&(data<=bB)] = bi
    return binned


def binFrameValues(data: np.ndarray,levels: int = 5) -> np.ndarray:
    '''
        Bin the data into the set number of levels and sets pixels to the
        index.
...
def binFrameVideo(data: np.ndarray,opath: str = None,levels: int = 5,mode: str = 'index'):
    '''
        Performs binFrameArray on each frame in the array, normalizes
        and writes the result to a video
    ...
    if mode not in ['index','values']:
        raise ValueError(f"Unsupported mode {mode}! Should be in ['index','values']")
    # if opath is None, set to a default value
    if opath is None:
...
def createTempDiffToVideo(data: np.ndarray,opath: str = None,cmap: int = cv2.COLORMAP_HOT):
    '''
        Creates a video from the absolute temperature difference between frames.
    ...
    # iterate over frames
    for fA,fB in zip(num_frames,num_frames[1:]):
        # get frame
        diff = np.abs(data[fA,:,:] - data[fB,:,:]) 
        # normalize
        diff -= diff.min()
        diff /= diff.max()
        diff *= 255.0
...
def findObjectsInTemp(data: np.ndarray):
    '''
...
class PlasmaGaussian:
    ...
    def __init__(self,lim=0.9):
        # if the limit is -ve raide error
        if lim <= 0:
            raise ValueError("Temperature limit cannot be 0! Received {lim}!")
        # store limit
        self._lim = lim
    ...
    def _clip(self,D):
        ...
        maxt = D.max()
        # make blank frame
        self._thresh = np.zeros(D.shape,dtype="uint8")
        # set pixels where the max temperature is above lim times max
        if self._rel:
            self._thresh[D>=(self._lim*maxt)] = 255
        else:
            self._thresh[D>=self._lim] = 255
        # if the threshold is all white then return early
        # this is usually when there's little going on in the frame so all the pixels are a similar temperature
        if (self._thresh==255).all():
            self._thresh[...] = 0
            return None,None
        ...
        self._ct = ct[ii,:,:]
        # return coordinates as row (y) column (x)
        return self._ct[:,:,1].flatten(),self._ct[:,:,0].flatten()

    def draw(self,draw_ct=False,draw_text=False,draw_gauss=False,draw_symmangle=False,**kwargs):
        '''
            Draw the results on an image

            The results stored from the previously run fit_guess are drawn on the thresholded temperature.

            The input flags control what is drawn.

            When draw_ct is True, then the filtered contour from the edge of the plasma is drawn. The colour of the line is set using ct_col.
            When draw_text is True, then a string of the fitted parameters is drawn. The colour of the text is set using text_col.
            When draw_gauss is True, then the filtered gaussian is drawn. The colour of the line is drawn using gauss_col.

            The default format of the draw text is:

                p= {height} {mean} and {std dev}

            A new formatting function can be passed via text_format keyword. Needs to accept the height, mean and std dev in that order.
        ...
        # draw found contour
        if draw_ct:
            self._draw = cv2.polylines(self._draw,[self._ct,],False,kwargs.get("ct_col",(0,0,255)),3)
        ...
        if draw_gauss and self._opt:
            ...
            # draw as a blue line
            self._draw = cv2.polylines(self._draw,[gct,],False,kwargs.get("gauss_col",(255,0,0)),2)
    ...
    def estRMSE(self):
        ...
        if not self._opt:
            return np.nan
        #x_axis = np.arange(self._ct[:,:,1].min(),self._ct[:,:,1].max(),1,dtype=np.int16)
        if self.__mode == "standard":
    ...
    def estSymmAngle(self,mid=None):
        '''
        ...
        if not self._opt:
            return np.nan
        # if there's only one point then ignore
    ...
    def estSkewKurtosis(self):
        '''
            Calculate the Skew, Kurtosis (Pearson) and Kurtosis (Fisher) and the last contour points as a gaussian
            distribution.
    ...
    def estAreaSplit(self,all_fill_value=5.0):
        '''
        ...
        if self._ct.shape[0]==1:
            return np.nan
        #full_area = cv2.contourArea(self._fullct)
        # find the median of the row values
        # the sort is to ensure it is sorted
        #mid = np.median(np.sort(self._fullct[:,:,1]))
        mid = self._fullct[:,:,1].min()+((self._fullct[:,:,1].max()-self._fullct[:,:,1].min())/2)
        #print(mid,self._fullct[:,:,1].max(),self._fullct[:,:,1].min())
        # split the contour to those ABOVE the mid point
        pt = self._fullct[self._fullct[:,:,1].flatten()>=mid,:,:]
        # find the area
        Aarea = cv2.contourArea(pt)
        # if it's a single or no data points then the area will be 0 raising an error
        if (Aarea==0) or (pt.shape[0]<=1):
            return all_fill_value
        #print("A ",pt.shape,Aarea)
        # split the contour to those BELOW the mid point
        pt = self._fullct[self._fullct[:,:,1].flatten()<=mid,:,:]
        Barea = cv2.contourArea(pt)
        if (Barea==0) or (pt.shape[0]<=1):
            return all_fill_value
        #print("B ",pt.shape,Barea)
        #print("Full ",cv2.contourArea(self._fullct))
        # return ratio between area
        return Aarea/Barea
            
    def fit_guess(self,D: np.ndarray) -> list:
        '''
            Assuming the plasma is a symmetrical gaussian distribution
            then the mean and std can be est from the coordinates without
            curve_fit
        ...
        y,x = self._clip(D)
        self._frame = cv2.normalize(D, None, 0, 255, cv2.NORM_MINMAX).astype("uint8")
    ...
    def fit_model(self, D: np.ndarray) -> list:
        ...
        popt,pcov=(curve_fit(PlasmaGaussian.fit_egauss, x, y))
        #fitted_curve=list(PlasmaGaussian.fit_egauss(x,popt[0],popt[1],popt[2]))
        self._opt = popt.tolist()
        return self._opt


    @staticmethod
    def fit_egauss(x, height, lbda, std_dev, mean):
        import scipy.special as sse
        # from https://stackoverflow.com/q/36900920
        # exponential gaussian
        return height*(0.5*lbda*np.exp(0.5*lbda*(2*mean+lbda*std_dev*std_dev-2*x))*sse.erfc((mean+lbda*std_dev*std_dev-x)/(np.sqrt(2)*std_dev)))


# generic method to use with CoaxialPlane.frame
def calcFrameMetric(frame,use_metric,segs):
    r,c = frame.shape
...
class CoaxialPlane:
    '''
        Estimating the coaxial plane gradient of the powder distribution by fitting a polynomial to the entropy of the temperature image.
    ...
    '''
    def __init__(self,k=1,nsegs=29,trim=True,line_lims=0,threshold=False,**kwargs):
        # order of fitting
        self._k = k
        ...
        if line_lims not in [0,1,2]:
            raise ValueError(f"Unsupported line limit mode {line_lims}!")
    ...
    def _checkInput(self,D):
        ...
        if (r==0) or (c==0):
            raise ValueError(f"Frame cannot be empty! Frame shape {r},{c}")
        # check if it fits in the height/rows
        if (r%self._segs)!=0:
            raise ValueError(f"Frame does not equally break up into {self._segs} segments along row axis!")
        # check if it fits in the width/columns
        if (c%self._segs)!=0:
            raise ValueError(f"Frame does not equally break up into {self._segs} segments along column axis!")
        # store image shape which is used later
        self._shape = [r,c]

    # break the frame into chunks and calculate entropy
    def frameEntropy(self,frame):
        '''
...
##                ent[self._segs*ri:self._segs*(ri+1),self._segs*ci:self._segs*(ci+1)] = entropy(np.unique(frame[self._segs*ri:self._segs*(ri+1),self._segs*ci:self._segs*(ci+1)],return_counts=True)[1],base=None)
    ...
    def drawLastLine(self,show_marks=False,on_source=False,**kwargs):
        '''
        ...
        if show_marks:
            ...
            coords = [(cc,rr) for cc,rr in zip(self._cw,self._rw)]
        ...
        for cA,cB in zip(coords,coords[1:]):
            draw = cv2.line(draw,cA,cB,kwargs.get("line_col",(255,0,0)),kwargs.get("line_width",3))
    ...
    def estTheta(self,offset=-np.pi/2):
        '''
        ...
        '''
        if len(self._last)==0:
            raise ValueError("Unable to find theta! Line has not been fitted")
        # create a poly using fitted parameters
        p = np.poly1d(self._last)
        # create X coordinates covering the x coordinates of fitted points
        if self._llims == 0:
    ...
    def estPlane(self,D):
        ...
        self._source = D
        if self.gray is None:
            self.gray = np.zeros(self._shape,D.dtype)
        # reset parameters
        self._last = [None for _ in range(self._k)]
        # calculate entropy
        ent = self.frameEntropy(D)
        # if entropy is all zero
        # return None
        if not ent.any():
            self._last = None
            return self._last
        # if there's some activity
        else:
            ...
            if self._thresh:
                ...
                self._rw,self._cw = np.where(self._threshMask==255)
    ...
    def estPlaneUsing(self,D,use_metric):
        '''
            Fit a line across the high entropy regions of the image.
    ...
    def getTempStats(self):
        ...
        thresh_data = self._source[self._threshMask==255]
    ...
    @staticmethod
    def _scaledSine(x,A,f,c):
        return (A*x*np.sin(x*(f/2*np.pi)))+c

    def estPlaneExperimental(self,D):
        '''
            Slightly different version of estPlane that attempts to fit a scaled sine wave
            to the entropy values using scipy.optimize.curve_fit function.
        ...
        # if there's some activity
            ...
            if self._thresh:
                ...
                # find which pixels are 255
                self._rw,self._cw = np.where(thresh==255)
            # if using the entropy as weights
            else:
                # convert gray to 0-1
                norm = self.gray.astype("float32")/255
    ...
    def drawLastExperimental(self,show_marks=False,on_source=False):
        #print(self._last)
        # make an image to draw the line on
        # either the previous source data colormapped
        if on_source:
...
def clipMaskPlasma(frame: np.ndarray,T: float = 1300,fill: str = 'min') -> np.ndarray:
    '''
    ...
    if frame.max()<T:
        ...
        if isinstance(fill,(int,float)):
            ...
            raise ValueError(f"Unsupported fill value {fill}! Has to be either a float, int or supported string")
        return frame
    ...
    while frame.max()>=T and (abs(i)<frame.shape[1]):
        # set fill value
        if isinstance(fill,(int,float)):
            frame[:,i] = fill
        elif fill == 'min':
...
def coaxialPlaneSlider(fn,k=2,filt=clipMaskPlasma):
    '''
    ...
    data = np.load(fn)["arr_0"]
    nf,r,c = data.shape
    cp = CoaxialPlane(k=k,trim=False,line_lims=1,min_segs=8,threshold=True)
    cv2.namedWindow("CP")
    if filt:
    ...
    def on_changed(val):
        ...
        # if there's a filtering function
        if filt:
            #print("orig ",frame.max())
...
def coaxialPlaneParams(fn,k=2,filt=None,**kwargs):
    '''
    ...
    for ni in range(nf):
        ...
        index.append(ni)
        time.append(ni/30)
    #print("hist: ",len(hist),len(hist[0]))
    # form into array for easier indexing
    hist = np.row_stack(hist)
    temp_stats = np.row_stack(temp_stats)
    #return hist
    #print("hist arr: ",hist.shape)
    #print("temp stats arr: ",temp_stats.shape)
    # make the columns for the poly coefficients
    poly_cols = [f"x^{i}" if i>0 else "offset" for i in range(hist.shape[1]-1,-1,-1)]
    df = pd.DataFrame({'Frame Index':index,'Time (s)':time} |
    ...
    if not kwargs.get("no_plot",False):
        ...
        f.savefig(f"{os.path.splitext(fn)[0]}-coaxial-plane-params-k{k}.png")
        ...
        ft.savefig(f"{os.path.splitext(fn)[0]}-coaxial-theta.png")
        return f,ft,df
    return _,_,df  # noqa: F821

def plasmaGaussianSlider(path,padding=10,**kwargs):
    '''
        Scroll through a file using a slider applying PlasmaGaussian.fit_guess to each frame
        and drawing the results
...
def plasmaGaussianParams(fn,transpose=2,**kwargs):
    '''
    ...
    df = pd.DataFrame({"Frame Index":index,"Time (s)":time,"Height (pixels)":hist[:,0],"Mean (pixels)":hist[:,1],"Std Dev (pixels)":hist[:,2],
    ...
    f.savefig(f"{os.path.splitext(fn)[0]}-gaussian-model-parameters.png")
    plt.close(f)
    ...
    dists = list(map(lambda x : distance.euclidean(x.flatten(),len(x)*[0,]),hist_s))
    ...
    fd.savefig(f"{os.path.splitext(fn)[0]}-parameter-euclidean-distance.png")
    plt.close(fd)
    ...
    dists = list(map(lambda a : distance.euclidean(a[0].flatten(),a[1].flatten()),zip(hist_s[::2],hist_s[1::2])))
    ...
    fd.savefig(f"{os.path.splitext(fn)[0]}-parameter-euclidean-distance-neighbours-p-2.png")
    plt.close(fd)

    ## windowed max distances
    # put the distances into a df
    C = pd.Series(list(map(lambda a : distance.euclidean(a[0].flatten(),a[1].flatten()),zip(hist_s,hist_s[1:]))))
    for w in [2,3,5]:
        # find the max distance to origin over windows of 3 samples
        D = C.rolling(w).max()
        fd,axd = plt.subplots(constrained_layout=True,figsize=[ww,hh])
        axd = D.plot(ax=axd,color=kwargs.get("plot_color","magenta"))
        ...
        fd.savefig(f"{os.path.splitext(fn)[0]}-windowed-parameter-euclidean-distance-neighbours-p-{w}.png")
        plt.close(fd)

        fd,axd = plt.subplots(constrained_layout=True,figsize=[ww,hh])
        Dp = D[D>0]
        if Dp.shape[0]==0:
            print(f"Skipping log scale for w={w} as all values are zero")
            continue
        axd = D.plot(ax=axd,color=kwargs.get("plot_color","magenta"))
        axd.set_yscale('log')
        axd.set(xlabel="Frame Number",ylabel="Rolling Max Distance",title=f"Max Rolling Parameter Euclidean\nDistance between Neighbours, w={w}")
        fd.savefig(f"{os.path.splitext(fn)[0]}-windowed-parameter-euclidean-distance-neighbours-p-{w}-pve-log.png")
        plt.close(fd)

    # plot the RMSE between the contour and structure 
    fe,axe = plt.subplots()
    axe.plot(rmse,color=kwargs.get("plot_color","magenta"))
    axe.set(xlabel="Frame Number",ylabel="RMSE (Contour vs Gaussian)",title=f"{os.path.splitext(os.path.basename(fn))[0]}\nRMSE between Contour and Gaussian Fitting")
    fe.savefig(f"{os.path.splitext(fn)[0]}-rmse.png")
    plt.close(fe)

    # plot the symmetric angle
    fs,axs = plt.subplots()
    axs.plot(symmangle,color=kwargs.get("plot_color","magenta"))
    axs.set(xlabel="Frame Number",title=f"{os.path.splitext(os.path.basename(fn))[0]}\nAngle Between Peak and Mid Point (rads)",ylabel="Angle (rads)")
    fs.savefig(f"{os.path.splitext(fn)[0]}-symmangle.png")
    plt.close(fs)
    ...
    fs.savefig(f"{os.path.splitext(fn)[0]}-area-ratio.png")
    plt.close(fs)
    ...
    fs.savefig(f"{os.path.splitext(fn)[0]}-skew.png")
    plt.close(fs)
    ...
    fs.savefig(f"{os.path.splitext(fn)[0]}-kurtosis-fisher.png")
    plt.close(fs)
    ...
    fs.savefig(f"{os.path.splitext(fn)[0]}-kurtosis-pearson.png")
    plt.close(fs)

    plt.close('all')
    return f,df

def plasmaAllGaussianParams(path,transpose=2,**kwargs):
    '''
    ...
    if plot_labels is None:
        plot_labels = [os.path.splitext(os.path.basename(fn))[0] for fn in glob(path)]
    # if the number of labels does not match the number of files
    # raise an error
    if len(plot_labels) != len(glob(path)):
        raise ValueError(f"Number of plot labels does not match the number of files! {len(plot_labels)} vs. {len(glob(path))}")
...
def plasmaGaussianBoxPlot(path,transpose=1,**kwargs):
    '''
    ...
    plot_labels = {fn:pl for fn,pl in zip([os.path.splitext(os.path.basename(fn))[0] for fn in glob(path)],plot_labels)}
    for aa in ax:
        if "xfsize" in kwargs:
            aa.tick_params(axis='x',labelsize=kwargs.get('xfsize'))
        if "yfsize" in kwargs:
    ...
    for fn,plabel in zip(glob(path),plot_labels):
        ...
        axc.plot(hist[:,0],hist[:,2],'x',label=fname)
        # add to history
    ...
    for pp,aa,title in zip([height_hist,std_hist],ax,["Height (pixels)","Std Dev (pixels)"]):
        ...
        #aa.set_xticklabels([plot_labels[int(xt)] for xt in aa.get_xticks()])
        aa.set_xticks(np.arange(1,len(pp.values())+1),labels=[plot_labels[k] for k in pp.keys()])
...
def temperatureStatsAboveLim(path,lim=1300.0,transpose=1,**kwargs):
    import os
    # get default figure size
    ww,hh = plt.rcParams["figure.figsize"]
    # create subplots scaling the image by number of required plots
...
def plasmaGaussianVideo(path,opath=None,**kwargs):
    '''
    ...
    if opath is None:
        opath = os.path.splitext(path)[0]+"-gaussian-params.avi"
    fourcc = cv2.VideoWriter_fourcc(*'mjpg')
    ...
    for ni in range(nf):
        frame = data[ni,:,:]
        cp.fit_guess(frame)
        draw = cp.draw(False,True,True,**kwargs)
        all_writer.write(draw)
    all_writer.release()

def stackPlasmaGaussianParams(path,transpose=2,**kwargs):
    '''
...
def plasmaGaussianScatterParamMP(path,target=0,**kwargs):
    import multiprocessing as mp
    if isinstance(path,str):
        path = glob(path)
    f,ax = plt.subplots()
    ...
    if (target < 0) or (target > 2):
        raise ValueError(f"Invalid target parameter index! Received {target}!")
    with mp.Pool(3) as pool:
        hist = pool.map(_getGPs,path)
    for fn,hh in zip(path,hist):
        ax.plot(hh[0],[x[target] for x in hh[1]],'x',label=os.path.splitext(os.path.basename(fn))[0])
    ax.legend()
...
def distanceToHotSlider(path):
    ''' GUI for showing distance to non-zero hottest spot '''
    data = np.load(path)["arr_0"]
    nf,r,c = data.shape
...
def coaxialPlaneSliderBlur(fn,k=2):
    '''
...
def videoCoaxialPlane(fn,k=2,opath=None,filt=None,mode=1,**kwargs):
    '''
    ...
    if opath is None:
        if (mode == 'min') or (mode == 0):
            opath = os.path.splitext(fn)[0]+f'_coaxial_plane_k_{k}_min{"" if filt is None else "-filt"}.avi'
            vc = c
            vr = r
        elif (mode == 'default') or (mode == 1):
            opath = os.path.splitext(fn)[0]+f'_coaxial_plane_k_{k}{"" if filt is None else "-filt"}.avi'
            vc = c*2
            vr = r
        elif (mode == 'grey') or (mode == 2):
            opath = os.path.splitext(fn)[0]+f'_coaxial_plane_k_{k}_grey-only{"" if filt is None else "-filt"}.avi'
            vc = c
            vr = r
        elif (mode == 'all') or (mode == 3):
            opath = os.path.splitext(fn)[0]+f'_coaxial_plane_k_{k}_all{"" if filt is None else "-filt"}.avi'
            vc = c*3
            vr = r
    ...
    if not writer.isOpened():
        raise OSError(f"Failed to open output file {opath}!")
    cp = CoaxialPlane(k=k,trim=False,line_lims=1,min_segs=8,threshold=False)
    for ni in range(nf):
        frame = data[ni,:,:]
        if isinstance(filt,(list,tuple)):
...
def videoCoaxialPlaneExperimental(fn,opath=None,filt=None,mode=1):
    '''
        Perform coaxial plane prediction on the target data and draw to a video file.
    ...
    if opath is None:
        if (mode == 'min') or (mode == 0):
            opath = os.path.splitext(fn)[0]+'_coaxial_plane_exp_min.avi'
            vc = c
            vr = r
        elif (mode == 'default') or (mode == 1):
            opath = os.path.splitext(fn)[0]+'_coaxial_plane_exp.avi'
            vc = c*2
            vr = r
        elif (mode == 'grey') or (mode == 2):
            opath = os.path.splitext(fn)[0]+'_coaxial_plane_exp_grey-only.avi'
            vc = c
            vr = r
        elif (mode == 'all') or (mode == 3):
            opath = os.path.splitext(fn)[0]+'_coaxial_plane_exp_all.avi'
            vc = c*3
            vr = r
...
def findSumTotalObjSize(data,units="pixels",**kwargs):
    '''
    ...
    # if the user wants it in metres squared
    elif units in ["m2","metres_sq"]:
        # get angles from keywords
        angleWidth = kwargs["angleWidth"]
        angleHeight = kwargs["angleHeight"]
        ...
        height_m = dist*np.tan(angleHeight)
        # use angle to convert image width to m
        width_m = dist*np.tan(angleWidth)
...
def recolormapData(temp,tmin=None,tmax=None,cmap='hot',mode='linear'):
    '''
    ...
    if isinstance(cmap,int):
        ...
        if cmap not in list(cmaps_cv2.values()):
            raise ValueError(f"Integer colormap code {cmap} not supported in cv2!")
        color_range = cmap
...
def maskMaxTempVideo(inf: str,outf: str = None,lim: float = 0.9):
    # if outf is None create it from the inf
    if outf is None:
        outf = os.path.splitext(os.path.basename(inf))[0]+".avi"
    # load input file
    ...
    if not writer.isOpened():
        raise ValueError(f"Failed to open video file {outf}!")
    # iterate over frames
...
def maskMaxTempSlider(path: str):
    '''
        Load a temperature NPZ file and use sliders to clip the temperature
        to identify hot areas
...
def drawLargestObjVideo(inf: str,outf: str = None,lim: float = 0.9):
     # if outf is None create it from the inf
    if outf is None:
        outf = os.path.splitext(os.path.basename(inf))[0]+"_lobj.avi"
    # load input file
    data = np.load(inf)["arr_0"]
    nf,r,c = data.shape
    # create the video writer
...
def drawExtBoundaryObj(inf: str,outf: str = None,lim: float = 0.9):
     # if outf is None create it from the inf
    if outf is None:
        outf = os.path.splitext(os.path.basename(inf))[0]+"_extct.avi"
    # load input file
    data = np.load(inf)["arr_0"]
    nf,r,c = data.shape
    # create the video writer
...
def plotExtBoundaryArea(inf: str,outf: str = None,lim: float = 0.9):
     # if outf is None create it from the inf
    if outf is None:
        outf = os.path.splitext(os.path.basename(inf))[0]+"_extct.png"
    # load input file
    data = np.load(inf)["arr_0"]
    nf,r,c = data.shape
    area = []
    ...
    f,ax = plt.subplots()
    ax.plot(area)
    ax.set(xlabel="Frame",ylabel="Outer Contour Area",title=f"{os.path.splitext(os.path.basename(inf))[0]} Sum External Area")
    f.savefig(outf)
    plt.close(f)
...
def slicTemperatureVideo(path: str,outf: str = None,segs: int = 1000):
    '''
        Iterate over each frame in a temperature NPZ file,
        perform SLIC to make a mask and write mask to video file
    ...
    '''
     # if outf is None create it from the inf
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+f"_slic_segs_{segs}.avi"
    # load input file
    if isinstance(path,str):
        data = np.load(path)["arr_0"]
    else:
...
def slicTemperatureColorVideo(path,outf=None,segs=1000):
    '''
        Iterate over each frame in a temperature NPZ file,
        perform SLIC to make a mask and write mask to video file
    ...
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+f"_color_slic_segs_{segs}.avi"
    # create the video writer
...
# classify regions of the image by statistical features
# using a KDTree to search for nearest matches
def classifyEqualAreas(frame: str,rsegs: int = 100,csegs=None):
    from scipy.spatial import distance
    r,c = frame.shape
    # decide how many segments there should be
    if (rsegs is None) and (csegs is not None):
        rsegs = csegs
    if (csegs is None) and (rsegs is not None):
        csegs = rsegs
    ri = np.linspace(0,r,rsegs,dtype="uint32")
    ci = np.linspace(0,c,csegs,dtype="uint32")
    feats = []
    # iterate over segments
    for rA,rB in zip(ri,ri[1:]):
        for cA,cB in zip(ci,ci[1:]):
            clip = frame[rA:rB,cA:cB]
            # collect statistics about regions of the image
            feats.append([clip.min(),
                          clip.max(),
                          clip.mean(),
                          clip.var()])
    # arrange the features into appropriate shape
    feats = np.row_stack(feats)
    # construct KD tree using the features
    tmp = distance.KDTree(feats)
    # for each segment find the closest matching other segments
    # removing them from the list each time


def denseOpticalFlow(path: str,outf: str = None,*args):
    if len(args)==0:
    ...
    ret,frameA = cap.read()
    if not ret:
        raise ValueError(f"Failed to read first frame from file {path}!")
    prevs = cv2.cvtColor(frameA,cv2.COLOR_BGR2GRAY)#
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+"_dense_optical_flow.avi"
    r,c,_ = frameA.shape
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        # convert to gray
        now = cv2.cvtColor(frameB, cv2.COLOR_BGR2GRAY)
        # perform optical flow looking at differences between current and prev frame
        flow = cv2.calcOpticalFlowFarneback(prevs, now, None, 0.5, 3, 15, 3, 5, 1.2, 0)
...
def denseOpticalFlowSlider(path: str,filt: list = None,winname: str = "optical-flow"):
    """
    ...
    def on_trackbar(val):
        ...
        ret,frameB = cap.read()
        if not ret:
            print(f"Failed to read frame {fi+1}!")
            return
        # apply given filters to frame
        if filt is not None:
            try:
        ...
        hsv[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
        bgr = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        cv2.imshow(winname,np.hstack((frameA,bgr)))
...
def trackHotPixel(path: str,outf: str = None):
    """
        ...
        os.path.splitext(os.path.basename(path))[0]+"_hot-pixel-marked.avi"
    ...
    print(nf,r,c)
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+"_hot-pixel-marked.avi"
    # create the video writer
    writer = cv2.VideoWriter(outf,cv2.VideoWriter_fourcc(*'mjpg'),30.0,(c,r),1)
...
def trackHotPixelHistory(path: str) -> plt.Figure:
    """
    ...
    for ni in range(nf):
        ...
        rhist.append(rm if frame[rm,cm]>1300 else None)
        chist.append(cm if frame[rm,cm]>1300 else None)
    f,ax = plt.subplots()
    ax.plot(chist,rhist,'r-')
    ax.set(xlabel='X Position',ylabel='Y Position')
    f.suptitle(f"{os.path.splitext(os.path.basename(path))[0]}, Hot Pixel Loc")
    return f

def denseOpticalFlowTemperature(path: str,outf: str = None,cmap: str = 'hsv',crop_plasma: bool = True,*args):
    '''
    ...
    hsv[..., 1] = 255
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+f"_dense_optical_flow_temp-{cmap}-{'-cropped-plasma' if crop_plasma else ''}.avi"
    # create the video writer
    ...
    for i in range(1,nf):
        # read first frame
        frameA = data[i-1,:,:]
        prvs = frame2gray(frameA)
        frameB = data[i,:,:]
        now = frame2gray(frameB)
        # perform optical flow looking at differences between current and prev frame
...
def denseOpticalFlowTemperatureQuiver(path: str,outf: str = None,*args):
    from matplotlib.animation import FuncAnimation
    # open soruce file
    data = np.load(path)["arr_0"]
    nf,r,c = data.shape
    ...
    ax[0].imshow(np.zeros_like(U,dtype="uint8"),cmap='hot')
    ax[1].set_aspect(ax[0].get_aspect())
    # set labels
    ax[0].set_title('Temperature (C)')
    ax[1].set_title('Optical Flow')
    # create quiver object to update
    Q = ax[0].quiver(X,Y,U,V,pivot='mid',scale=1e4,cmap='hot')

    # loop collecting frames
    def update_quiver(num,Q,X,Y):
        frameA = data[max(0,num-1),:,:]
        prvs = frame2gray(frameA)
        frameB = data[num,:,:]
        now = frame2gray(frameB)
        # perform optical flow looking at differences between current and prev frame
        ...
        mag, ang = cv2.cartToPolar(flow[..., 0], flow[..., 1])
        Q.set_UVC(U,V)
        ax[0].imshow(frameB,cmap='hot')
        #prevs = now
        return Q,
    if outf is None:
        outf = os.path.splitext(path)[0]+"_dense_optical_flow_temp-plt.avi"
    f.tight_layout()
    anim = FuncAnimation(f,update_quiver,frames=np.arange(nf),fargs=(Q,X,Y),interval=int((1/30)*1000),blit=False)
    anim.save(outf)

def denseOpticalFlowTemperatureQuiverSlider(path: str,hide_temp: bool = False,clip_plasma: bool = True,flow_args: list = []):
    # open soruce file
    data = np.load(path)["arr_0"]
    if clip_plasma:
        data = data[:,:,:360]
    ...
    # create initial one to set aspect ratio
    if not hide_temp:
        ax[0].imshow(np.zeros_like(U,dtype="uint8"))
        #ax[1].set_aspect(ax[0].get_aspect())
        # set labels
        ax[0].set_title('Temperature (C)')
        ax[1].set_title('Optical Flow')
        ...
        Q = ax[1].quiver(X,Y,U,V,pivot='mid',scale=1e2)
        ...
        for aa in ax:
            ...
            aa.set_aspect('equal')
        # create quiver object to update
        Q = ax[0].quiver(X,Y,U,V,pivot='mid',scale=1e2,cmap='hot')
    ...
    print(np.unravel_index(np.argmax(data),(nf,r,c)))
    freq_slider = Slider(
        ax=axfreq,
        label='Frame',
        valmin=0,
    ...
    def update_quiver(val):
        ...
        #print(V.min(),V.max())
        mag, ang = cv2.cartToPolar(flow[..., 0], flow[..., 1])
        ## mask values according to entripy
        #ent = cp.frameEntropy(frameA)
        #ent = frame2gray(ent)
        #U[ent==0] = np.nan
        #V[ent==0] = np.nan
        ## make values by temperature
        #U[frameB < 0.85*frameB.max()] = None
        #V[frameB < 0.85*frameB.max()] = None
        ## mask values by most active directions
        #U[U < 0.1* U.max()] = 0
        #V[V < 0.1* V.max()] = 0
        ## mask and norm
        U[U==0] = None
        V[V==0] = None
        print(num,"U: ",np.nanmin(U),np.nanmax(U),"V: ",np.nanmin(V),np.nanmax(V))
        ...
        if not hide_temp:
            ...
            for coll in ax[2].collections:
                #for a in coll:
                #    a.remove()
                #ax[2].collections.remove(coll)
                coll.remove()
            ax[2].contourf(U,cmap='hot')
            for coll in ax[3].collections:
                ax[3].collections.remove(coll)
            ax[3].contourf(V,cmap='hot')
        else:
            for coll in ax[1].collections:
                coll.remove()
                #ax[1].collections.remove(coll)
            ax[1].contourf(U,cmap='hot')
            for coll in ax[2].collections:
                coll.remove()
                #ax[2].collections.remove(coll)
            ax[2].contourf(V,cmap='hot')
        #prevs = now
        f.canvas.draw_idle()
...
def denseOpticalFlowTemperatureStreamSlider(path: str,hide_temp: bool = False,flow_args: list=[]):
    from matplotlib.widgets import Slider
    ...
    # create initial one to set aspect ratio
    if not hide_temp:
        ax[0].imshow(np.zeros_like(X,dtype="uint8"))
        #ax[1].set_aspect(ax[0].get_aspect())
        # set labels
        ax[0].set_title('Temperature (C)')
        ax[1].set_title('Optical Flow')
...
def denseOpticalFlowTemperatureSlider(path: str,filt: np.ndarray = None,crop_plasma: bool = False,**kwargs):
    """
...
def videoTemperatureFFT(path: str,outf: str = None,use_log_mag: bool = True):
    ...
    print(nf,r,c)
    if outf is None:
        outf = os.path.splitext(os.path.basename(path))[0]+"_fft_temp.avi"
    # read first frame
    ...
    writer = cv2.VideoWriter(outf,cv2.VideoWriter_fourcc(*'mjpg'),30.0,(c,r),0)
    if not writer.isOpened():
        raise OSError(f"Failed to open output file {outf}!")
    #spacer = np.zeros((r,spacer),dtype="uint8")
    writer.write(frame)
    # iterate over each frame
    for f in range(1,nf):
        out = fft2(temp[f,:,:])
...
def videoTemperatureEntropy(path: str,opath: str = None,segs: int = 29):
    '''
    ...
    from scipy.stats import entropy
    if opath is None:
        opath = os.path.splitext(os.path.basename(path))[0]+'_entropy_mask.avi'
    # load data
    temp = np.load(path)["arr_0"]
    # get number of frames
    nf,r,c = temp.shape
    ...
    if not out.isOpened():
        raise ValueError(f"Failed to open output file at {opath}!")
    ...
    for ni in range(nf):
        ...
        #frame_rgb = plt.cm.hot(norm(frame))
        ent = frame_entropy(frame)
        # convert entropy to grayscale
        #ent_gray = 255.0*plt.cm.gray(norm(ent))
        ent_gray = frame2gray(ent)
        #print(ent_gray.shape,ent_gray.dtype,ent_gray.min(),ent_gray.max())
        ...
        #frame_mask = plt.cm.hot(norm(frame_mask))
...
def findOrderedStripes(path: str,order: str = "top-bottom",as_ct: bool = True) -> list:
    ...
    if len(cts)==0:
        raise ValueError("Failed to find contours!")
    # search for where hier is -1
    ci = np.where(hier[0,:,-1]==-1)[0]
    # check if any match
    if ci.shape[0]==0:
        raise ValueError("Cannot find any contours whose hierarchy is -1!")
    # get contours
    ...
    if order == "top-bottom":
        ...
        raise ValueError(f"Unsupported target order! Received {order}!")
    # return ordered stripes as contour paths
    if as_ct:
        return cts
    else:
...
def plotMaxStripeTemperature(path: str,mask: str,**kwargs) -> plt.Figure:
    '''
    ...
    pks = []
    for (mi,mask),L in zip(enumerate(masks_list),stripe_labels):
        data_mask = data.copy()
        # brute force masking
        data_mask[:,mask!=255] = 0
        # get max temperature within region
        maxt = data_mask.max((1,2))
        ax.plot(x,maxt,label=L)
        if kwargs.get("clip",False):
            pks+= find_peaks(maxt,height=0.9*maxt.max())[0].tolist()
    ax.legend()
    ax.set(xlabel="Frame Number",ylabel="Max Frame Temperature ($^\circ$C)")
    # attempt to clip to activity
    if kwargs.get("clip",False):
        ax.set_xlim(min(pks)-100,max(pks)+100)
    f.suptitle(kwargs.get("title",f"{os.path.splitext(os.path.basename(path))[0]} Stripe Temperatures"))
    return f
...
def gaussianMixtureHist(path: str,nb: int = 60):
    ...
    def on_changed(val):
        ...
        # get frame
        frame = data[ni,:,:]
        # calculate histogram of of the frame
        #hist,bin_edges = np.histogram(frame,bins=nb)
        # find the centres of each bin
...
def temperatureFlatten(path: str):
    ...
    X = np.arange(0,c,1,dtype="uint16")
    Y = np.arange(0,r,1,dtype="uint16")
    X,Y = np.meshgrid(X,Y)
    ...
    ax[0].contourf(X,Y,data[0,:,:],cmap='hot')
    ...
    def update(val):
        ...
        frame = data[ni,:,:]
        ax[0].contourf(X,Y,frame,cmap='hot')
        # update line plot
...
def morphCorrectPlate(img: np.ndarray,corners: list = [[162,52],[376,0],[376,348],[154,297]]) -> np.ndarray:
    """
    ...
    #targets = [(107,0),(376,0),(376,348),(117,348)]
    maxW = max([x[0] for x in corners])
    maxH = max([x[1] for x in corners])
    targets = np.float32([(0,0),(maxW-1,0),(maxW-1,maxH-1),(0,maxH-1)])
    # get perspective transform
    M = cv2.getPerspectiveTransform(np.float32(corners),np.float32(targets))
    # warp setting target size based on targets
    return cv2.warpPerspective(img,M,(maxH,maxW),flags=cv2.INTER_LINEAR)


def morphCorrectPlateVideo(path: str,opath: str = None,corners: list = [[162,52],[376,0],[376,348],[152,310]]):
    """
    ...
    if not video.isOpened():
        raise ValueError(f"Failed to open file {path}!")
    if opath is None:
        opath = os.path.splitext(os.path.basename(path))[0]+'-plate-warp.avi'
    ...
    while ret:
        ...
        #print(frame.shape,morph.shape)
        #stack = cv2.hconcat([frame,morph])
        if out is None:
...
def convolveColor(path: str,force_odd: bool = True, kernel = None):
    ...
    if not source.isOpened():
        raise ValueError(f"Cannot open source file {path}!")
    ...
    def on_changed(val):
        ...
        if not ret:
            print(f"Failed to read frame {ni}!")
            return
        # get filter size
        ...
        #res = frame2gray(cv2.Sobel(frame,cv2.CV_64F,1,0,ksize=r))
...
def convolveTemperature(path: str, force_odd: bool = True, kernel: np.ndarray = None):
    ...
    def on_changed(val):
        ...
        print(frame.min(),frame.max(),res.min(),res.max())
        # colormap
...
def combineEmissivity(pathA: str,pathB: str,th: float) -> np.ndarray:
    '''
        Combine two temperature files with different emissivities

        Values in pathA
    '''
    # load files
    dataA = np.load(pathA)["arr_0"]
    dataB = np.load(pathB)["arr_0"]
    dataC = dataA.copy()
    dataC[dataA>=th] = dataB[dataA>=th]
    return dataC


def formatEstAllPlasmaSizeToDF(data: dict) -> pd.DataFrame:
    '''
        Formats the nested dict from estAllPlasmaSize into a pandas dataframe
...
def plotMaxContourLW(data_df: pd.DataFrame,th: str = 'all',**kwargs) -> list[plt.Figure]:
    ...
    for t in th:
        ...
        tax.grid(False)
        # set title
        ax.figure.suptitle("Max Plasma Contour Length and Width (Tlim$\geq$"+f"{t}"+"$^\circ$C)")
        # make legend
...
def plotContourAreaVsTime(data_df: pd.DataFrame) -> list[plt.Figure]:
    figs = []
    for th in data_df['Threshold (C)'].unique():
        ...
        f.suptitle("Estimated Plasma Area vs Time (Tlim$\geq$"+f"{th}"+"$^\circ$C)")
        figs.append(f)
    return figs


def plotAsymmetricalAngle(plasma_all_params: pd.DataFrame,agg_fn=np.mean,drop_file: int = -1,group: str = 'File') -> plt.Figure:
...
def maskTemp(x,lim=0.15):
    #print(x.min(),x.max(),f"lim {lim*x.max()}")
    x[x>=(lim*x.max())]=x.min()
    return x


def clipShape(x,cclip=None,rclip=None):
    r,c = x.shape
    if cclip:
        x = x[:,:min([cclip,c])]
    if rclip:
...
def maskShape(x,cclip=None,rclip=None):
    r,c = x.shape
    xmin = x.min()
    if cclip:
        x[:,min([cclip,c]):] = xmin
...
def collectFolderAllPlasmaParams(path: str,tlim: float = 1000.0,**kwargs) -> pd.DataFrame:
    ...
    for fn in path:
        ...
        df['File'] = os.path.splitext(os.path.basename(fn))[0]
        #df['Label'] = label
...
##    combined = pd.concat(all_dfs,ignore_index=True)
    ...
    #coaxialPlaneSlider(glob(r"powder_plasma_npz/npz/*.npz")[1],k=5)
...
##        break
##        f.savefig(f"{os.path.splitext(fn)[0]}-gaussian-plasma-params.png")
##        plt.close(f)
...
##            f.savefig(f"{os.path.splitext(fn)[0]}-coaxial-plane.png")
##            plt.close(f)
##            ft.savefig(f"{os.path.splitext(fn)[0]}-coaxial-theta.png")
##            plt.close(ft)
##            ft.axes[0].set_yscale('log')
##            ft.savefig(f"{os.path.splitext(fn)[0]}-coaxial-theta-log.png")
...
##        mask = glob(f"stripes_masks/cleanup{os.path.splitext(os.path.basename(fn))[0]}*stripes.jpg")
##        if len(mask)>0:
##            f = plotMaxStripeTemperature(fn,mask[0],stripe_labels=stripe_labels[os.path.splitext(os.path.basename(fn))[0]],title=f"Stripe Temperature ($^\circ$C)")
##            f.savefig(f"{os.path.splitext(fn)[0]}-stripe-max-temps.png")
##            plt.close(f)
##            f = plotMaxStripeTemperature(fn,mask[0],clip=True,title=f"Stripe Temperature ($^\circ$C)",stripe_labels=stripe_labels[os.path.splitext(os.path.basename(fn))[0]])
##            f.savefig(f"{os.path.splitext(fn)[0]}-stripe-max-temps-clipped.png")
##            plt.close(f)
##    plt.show()
...
##        if os.path.splitext(os.path.basename(fn))[0][-1] == '1':
##            continue
##        f = plasmaGaussianParams(fn,True)
##        f.savefig(f"{os.path.splitext(os.path.basename(fn))[0]}-gaussian-plasma-params-flip.png")
##        plt.close(f)
##    search = glob("powder_plasma_npz/npz/*.npz")
##    # remove specific files we know not to contain plasma
##    # 1,3
##    for fi,fn in enumerate(search):
##        if os.path.splitext(os.path.basename(fn))[0][-1] in ['1','3']:
##            search.remove(fn)
...
##        denseOpticalFlowTemperatureQuiver(fn,None,20/100,1,21,1,1,1.0,cv2.OPTFLOW_FARNEBACK_GAUSSIAN)
    ...
    #    f = trackHotPixelHistory(fn)
        #f.savefig(f"{os.path.splitext(os.path.basename(fn))[0]}-hot-pixel-location.png")
        #plt.close(f)
